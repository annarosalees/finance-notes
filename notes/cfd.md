...
- [x] レートはどう生成されるか
- [ ] カバーディールの考え方
  <!-- 着手時は HANDOFF_cover-deal-memo.txt（Git管理対象外・リポジトリ直下）
       を参照。相対取引の3層構造（顧客ポジション→対顧客ポジション→
       ヘッジポジション）とデルタニュートラルの下書き素材あり
       （2026-09-21、Issue #60の壁打ち中に退避）。 -->
...
- 決済のreconciliation（突合）：顧客の決済結果が、社内システムとカ
  バー先（LP/PB）側の記録の両方で正しく一致しているかを確認する。
...
- レートの異常値監視（アボート）：市場価格が急に跳ね上がる（または落
  ...
  なお、顧客向けに生成しているレートと、カバー先（LP）に流すレートは
...
「カバー」は、顧客の注文をカバー先（LP）にどう流すかという、別の業務
・別のプロセスである。
